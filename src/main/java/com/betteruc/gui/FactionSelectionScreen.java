...
public class FactionSelectionScreen extends Screen {
    ...
    protected void init() {
        ...
        addDrawableChild(ButtonWidget.builder(Text.literal(toggleLabel()), b -> {
            BetterUCConfig.toggleTrackedFaction(selectedFactionQuery);
            BetterUCConfig.rebuildRemoteFactionUnion();
            if (BetterUCConfig.isFactionTracked(selectedFactionQuery)) {
        ...
        if (dropdownOpen) {
            y += ROW_STEP;
            for (BetterUCConfig.TrackableFaction faction : BetterUCConfig.getTrackableFactions()) {
                final String query = faction.query;
                addDrawableChild(ButtonWidget.builder(Text.literal(optionLabel(faction)), b -> {
                    selectedFactionQuery = BetterUCConfig.normalizeFactionQuery(query);
                    BetterUCConfig.setOnlyTrackedFaction(selectedFactionQuery);
                    BetterUCConfig.rebuildRemoteFactionUnion();
                    requestSilentMemberRefresh(selectedFactionQuery);
                    dropdownOpen = true;
                    refreshWidgets();
    ...
    private String dropdownLabel() {
        String label = BetterUCConfig.factionLabelForQuery(selectedFactionQuery);
        return "\u00A7bFraktion: \u00A7f" + label + (dropdownOpen ? " \u25B2" : " \u25BC");
    }

    private String toggleLabel() {
        boolean tracked = BetterUCConfig.isFactionTracked(selectedFactionQuery);
        return tracked ? "\u00A7aTracking: AN" : "\u00A77Tracking: AUS";
    }

    private String optionLabel(BetterUCConfig.TrackableFaction faction) {
        boolean tracked = BetterUCConfig.isFactionTracked(faction.query);
        return (tracked ? "\u00A7a[AN] " : "\u00A77[AUS] ") + faction.label;
    }

    private void ensureSelectedFaction() {
        List<String> tracked = BetterUCConfig.getTrackedFactionQueries();
        if (!tracked.isEmpty()) {
            selectedFactionQuery = tracked.get(0);
            return;
        }
        String normalizedCurrent = BetterUCConfig.normalizeFactionQuery(selectedFactionQuery);
        if (!normalizedCurrent.isEmpty()) {
            selectedFactionQuery = normalizedCurrent;
            return;
        }
        selectedFactionQuery = BetterUCConfig.normalizeFactionQuery("kartell");
    }

    private void requestSilentMemberRefresh(String factionQuery) {
        SyncRefreshActions.requestFactionRefresh(client, factionQuery, false);
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawCenteredTextWithShadow(
                textRenderer,
                Text.literal("\u00A77Aktiv: \u00A7f" + tracked.stream().map(BetterUCConfig::factionLabelForQuery).reduce((a, b) -> a + ", " + b).orElse("keine")),
                centerX,
                34,
                0xBBBBBB
        );
