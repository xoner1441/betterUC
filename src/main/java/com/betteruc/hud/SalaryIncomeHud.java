...
/** Tracks salary income that is waiting to be paid out at the next PayDay. */
public final class SalaryIncomeHud {
    ...
    private static final Pattern SALARY_INCOME_PATTERN = Pattern.compile(
            "(?iu)^\\[payday]\\s+du\\s+bekommst\\s+dein\\s+gehalt\\s+von\\s+"
    ...
    private SalaryIncomeHud() {
    }

    public static void register() {
        restoreFromConfig();
        HudElementRegistry.addLast(
                Identifier.fromNamespaceAndPath("betteruc", "salary_income"),
                (context, tickCounter) -> {
    ...
    public static boolean updateFromChatLine(String raw) {
        Long amount = parseSalaryIncome(raw);
    ...
    static Long parseSalaryIncome(String raw) {
        if (raw == null || raw.isBlank()) return null;
        String cleaned = stripChatPrefix(stripFormatting(raw));
        Matcher matcher = SALARY_INCOME_PATTERN.matcher(cleaned);
    ...
    private static void render(GuiGraphicsExtractor context) {
        ...
        if (client.player == null || !BetterUCConfig.INSTANCE.showSalaryIncomeHud
                || !shouldShow(currentIncome)) return;

        String value = formatMoney(currentIncome) + "$";
        String style = BetterUCConfig.INSTANCE.salaryIncomeHudStyle;
        String displayText = BetterUCConfig.prefixedHudText(
                BetterUCConfig.INSTANCE.salaryIncomeHudPrefixEnabled,
                BetterUCConfig.INSTANCE.salaryIncomeHudPrefix,
                value
        );
        ...
        ModernHudRenderer.drawScaledWithGradient(
                context,
                BetterUCConfig.INSTANCE.salaryIncomeHudX,
                BetterUCConfig.INSTANCE.salaryIncomeHudY,
                BetterUCConfig.INSTANCE.salaryIncomeHudScale,
                BetterUCConfig.INSTANCE.salaryIncomeHudGradientEnabled,
                BetterUCConfig.INSTANCE.salaryIncomeHudGradientColor,
                () -> {
                    if (BetterUCConfig.isStylizedHudStyle(style)) {
                        ModernHudRenderer.drawStyledText(
                                ...
                                client,
                                style,
                                BetterUCConfig.INSTANCE.salaryIncomeHudCustomFont,
                                displayText,
                                0,
                                0,
    ...
    private static void restoreFromConfig() {
        currentIncome = Math.max(0L, BetterUCConfig.INSTANCE.lastKnownSalaryIncome);
    }

    private static void persist() {
        long safeIncome = Math.max(0L, currentIncome);
        if (BetterUCConfig.INSTANCE.lastKnownSalaryIncome == safeIncome) return;
        BetterUCConfig.INSTANCE.lastKnownSalaryIncome = safeIncome;
        BetterUCConfig.save();
    }
