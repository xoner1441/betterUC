...
public class EigenbedarfConfigScreen extends Screen {
    ...
    protected void init() {
        for (int i = 0; i < ROWS; i++) {
            KartellConfig.EigenbedarfPreset preset = getPreset(i);
            drugIndex[i] = findDrugIndex(preset.droge);
            purity[i] = KartellConfig.clampEigenbedarfPurity(preset.reinheit);
            addRow(i);
        }
    ...
    private String drugButtonLabel(int row) {
        return "Droge: " + KartellConfig.EIGENBEDARF_DRUG_OPTIONS[drugIndex[row]];
    }
    ...
    private void cycleDrug(int row) {
        drugIndex[row] = (drugIndex[row] + 1) % KartellConfig.EIGENBEDARF_DRUG_OPTIONS.length;
    }
    ...
    private int findDrugIndex(String raw) {
        String normalized = KartellConfig.normalizeEigenbedarfDrug(raw);
        for (int i = 0; i < KartellConfig.EIGENBEDARF_DRUG_OPTIONS.length; i++) {
            if (KartellConfig.EIGENBEDARF_DRUG_OPTIONS[i].equalsIgnoreCase(normalized)) {
                return i;
            }
        }
        return 0;
    ...
    private KartellConfig.EigenbedarfPreset getPreset(int row) {
        if (row == 0) {
            if (KartellConfig.INSTANCE.eigenbedarfSlot1 == null) {
                KartellConfig.INSTANCE.eigenbedarfSlot1 = new KartellConfig.EigenbedarfPreset(
                        KartellConfig.EIGENBEDARF_DRUG_OPTIONS[0], 0, 0
                );
            }
            return KartellConfig.INSTANCE.eigenbedarfSlot1;
        }

        if (KartellConfig.INSTANCE.eigenbedarfSlot2 == null) {
            KartellConfig.INSTANCE.eigenbedarfSlot2 = new KartellConfig.EigenbedarfPreset(
                    KartellConfig.EIGENBEDARF_DRUG_OPTIONS[1], 0, 0
            );
        }
        return KartellConfig.INSTANCE.eigenbedarfSlot2;
    }

    private void saveAndClose() {
        savePreset(0, KartellConfig.INSTANCE.eigenbedarfSlot1);
        savePreset(1, KartellConfig.INSTANCE.eigenbedarfSlot2);
        KartellConfig.save();
        if (client != null) {
    ...
    private void savePreset(int row, KartellConfig.EigenbedarfPreset target) {
        ...
        target.droge = KartellConfig.normalizeEigenbedarfDrug(KartellConfig.EIGENBEDARF_DRUG_OPTIONS[drugIndex[row]]);
        target.menge = KartellConfig.clampEigenbedarfAmount(amount);
        target.reinheit = KartellConfig.clampEigenbedarfPurity(purity[row]);
    }
