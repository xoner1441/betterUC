...
public class BankBalanceHud {
    ...
    private static void render(DrawContext context) {
        ...
        if (!KartellConfig.INSTANCE.showBankHud) return;
        if (currentBankBalance < 0) return;

        int x = KartellConfig.INSTANCE.bankHudX;
        int y = KartellConfig.INSTANCE.bankHudY;
        String text = "Bank: " + formatMoney(currentBankBalance) + "$";
        context.drawTextWithShadow(client.textRenderer, Text.literal(text), x, y, KartellConfig.INSTANCE.bankHudColor);
    }
    ...
    private static void restoreFromConfig() {
        currentBankBalance = Math.max(-1, KartellConfig.INSTANCE.lastKnownBankBalance);
    }

    private static void setBalanceAndPersist(int newBalance) {
        if (newBalance < 0) return;
        boolean changed = currentBankBalance != newBalance
                || KartellConfig.INSTANCE.lastKnownBankBalance != newBalance;
