  ...
  wrapper.innerHTML = `
    ...
    <section class="download-dialog" role="dialog" aria-modal="true" aria-labelledby="downloadModalTitle">
      ...
      <div class="download-dialog-body">
        <div class="download-divider"><span>Aktuelle Spielversion</span></div>
        ...
        <p class="download-modal-hint" id="downloadModalHint">Die mc26.x-JAR aktualisiert auch ältere betterUC-Installationen aus der 26.x-Zeit.</p>
      </div>
    </section>
  `;
...
function openDownloadModal() {
  ensureDownloadModal();
  selectedDownloadVersion = DOWNLOAD_VERSIONS[0];
  updateDownloadModalState();
  downloadModal.hidden = false;
  document.body.classList.add("modal-open");
  refreshSelectedDownloadRelease(selectedDownloadVersion);
  downloadModal.querySelector("#downloadResultButton")?.focus();
}
...
function updateDownloadModalState(releaseInfo = null) {
  ...
  if (!selectedDownloadVersion) {
    ...
    if (hint) hint.textContent = "Der Download für Minecraft 26.2 wird vorbereitet.";
    return;
  }

  const downloadUrl = "/download/latest-mc26.x.jar";
